...
function processNewRegattaSheets() {
  ...
  // Skip already processed
    ...
    if (file.getMimeType() === MimeType.GOOGLE_SHEETS) {
      ...
      parsed = parseRaceResultsFromImage(file);
    ...
    //try {
      ...
      const sheetID = roundWrite(overallSheetID, rankedScoresMap, parsed);

      // 5. Add round to the Overall sheet (and trigger series recalc)
      appendRound(overallSheetID, parsed, rankedScoresMap);;

      // 6. Facebook post processing
      postRoundResults(parsed, overallSheetID, sheetID)
...
function finalizeRaceResultsFile(file, parsed) {
  ...
  const newDescription = `Processed by SMMC Admin AI`;
  try {
