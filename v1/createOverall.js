...
function getOrCreateOverall(regattaName, parsed, members) {
  ...
  overallSetup(newFile, parsed, members);
...
function overallSetup(bookID, parsed, members) {
  ...
  // Create Config Sheet (and hide it)
  let configSh = ss.getSheetByName("Config") || ss.insertSheet("Config");
  configSh.hideSheet();
  if (configSh.getLastRow() === 0) {
    configSh.getRange("A1:C1").setValues([["eventID", "roundLabel", "colIndex"]]);
  }
